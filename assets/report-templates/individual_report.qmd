...
for (clustered_reads_file in list.files(".", pattern="meta_id_.*_CGIX_clustered_reads.tsv.gz")) {
    clustered_reads <- read_tsv(clustered_reads_file, show_col_types=FALSE)
    ...
    cat(paste0(clustered_reads_file, ": ", num_clustered_reads, "\n"))
}
```
...
for (tissue in all_tissues){
    temp <- read_tsv(paste0(patient,"_",tissue,"_CGIX_skew.tsv.gz")) %>% mutate(patient = patient, tissue = tissue, folded_skew = ifelse(H1_Xa_skew > 0.5, 1 - H1_Xa_skew, H1_Xa_skew), reads_in_block = H1_Xa + H2_Xa + H1_Xi + H2_Xi)
...
H1_all_binned <- H1_all %>%
    ...
    summarise(reads_in_block_sum = sum(reads_in_block, na.rm = TRUE))
#now folded
...
p <- ggplot(H1_all, aes(x = H1_Xa_skew, y = tissue, fill = tissue)) + 
    geom_density_ridges() + 
    labs(title = "H1 skew", x = "skew", y = "tissue") + 
    theme_half_open() + 
    background_grid() + 
